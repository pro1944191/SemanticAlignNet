...
if __name__ == '__main__':
    ...
    #width = int(360 / 360 * 512)

    # define placeholders
    grd_x = tf.compat.v1.placeholder(tf.float32, [None, 128, width, 3], name='grd_x')
    sat_x = tf.compat.v1.placeholder(tf.float32, [None, 256, 256, 3], name='sat_x')
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        """load_model_path = './Model/polar_' + str(polar) + '/' + data_type + '/' + network_type \
                          ...
                          + '/model.ckpt'"""
        #load_model_path = './Model/TRY_polar_1/CVUSA/VGG_13_conv_v2_cir/train_grd_noise_360/train_grd_FOV_CUSTOMstr(train_grd_FOV)/FOV/14/model.ckpt'
        #load_model_path = './Model/TRY_polar_1/CVUSA/VGG_13_conv_v2_cir/train_grd_noise_360/train_grd_FOV_ADAM_1e4str(train_grd_FOV)/FOV/34/model.ckpt'
        load_model_path = './Model/TRY_polar_1/CVUSA/VGG_13_conv_v2_cir/train_grd_noise_360/train_grd_FOV_ADAM_1e3_PADDING_IMAGES_NO_NORMALIZATIONstr(train_grd_FOV)/FOV/21/model.ckpt'
        ...
        while True:
            ...
            print(grd_matrix_val.shape)
            print(orientation_gth.shape)
        ...
        file = './Result/CVUSA/Descriptor/' \
               ...
               + 'test_grd_noise_' + str(test_grd_noise) + '_test_grd_FOV_' + str(test_grd_FOV) + 'DEBUG' \
               + '_' + network_type + '.mat'
