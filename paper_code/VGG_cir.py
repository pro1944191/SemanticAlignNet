...
#            self.layer17_output = tf.nn.dropout(self.layer17_output, keep_prob, name='conv5_3_dropout')
    ...
    def conv2d(self, x, W):
        # w_pad = tf.pad
        return tf.nn.conv2d(input=x, filters=W, strides=[1, 1, 1, 1],
                            padding='VALID')
    ...
    def conv_layer2(self, x, kernel_dim, strides, input_dim, output_dim, trainable, activated,
        ...
        with tf.compat.v1.variable_scope(name): # reuse=tf.AUTO_REUSE
            ...
            conv = tf.nn.conv2d(input=x, filters=weight, strides=strides, padding='VALID')
...
"""def warp_pad_columns2(self, x, n=1):
...
batch_grd[0,:,:,:] = img
out = tf.concat([batch_grd[:, :, -1:, :], batch_grd, batch_grd[:, :, :1, :]], axis=2)
print(tf.pad(tensor=out, paddings=[[0, 0], [1, 1], [0, 0], [0, 0]]))"""

img = cv2.imread('./Data/CVUSA/bingmap/19/0000001.jpg')
img = cv2.resize(img, (512, 128), interpolation=cv2.INTER_AREA)
batch_grd = np.zeros([1, 128, 512, 3], dtype = np.float32)
batch_grd[0,:,:,:] = img
model = VGG16_cir(batch_grd,0.8, False, 'VGG_sat')
