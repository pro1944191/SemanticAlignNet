...
class InputData:
    ...
    def next_batch_scan(self, batch_size, grd_noise=360, FOV=360):
        ...
        for i in range(batch_size):
            ...
            # ground
            img = cv2.imread((self.img_root + self.id_test_list[img_idx][2])) #.replace("streetview","streetview_180_noshift")
...
#        print(grd_shift[0])
    ...
    def next_pair_batch(self, batch_size, grd_noise=360, FOV=360):
        ...
        #print("grd width: {}".format(grd_width))
        batch_sat_polar = np.zeros([batch_size, 128, 512, 3], dtype=np.float32)
        batch_sat = np.zeros([batch_size, 256, 256, 3], dtype=np.float32)
        batch_grd = np.zeros([batch_size, 128, grd_width, 3], dtype=np.float32)
        grd_shift = np.zeros([batch_size,], dtype=np.int_)
        ...
        while True:
            ...
            """if count == 0:
                grd_width = int(360/360*512)
            if count == 4:
                grd_width = int(180/360*512)
            if count == 8:
                grd_width = int(90/360*512)
            if count == 12:
                grd_width = int(70/360*512)"""
            ...
            #print((self.img_root + self.id_list[img_idx][2]).replace("streetview","streetview_180_noshift"))
            #print("-------------------------------------")
            img = cv2.imread((self.img_root + self.id_list[img_idx][2]).replace("streetview","streetview_180_noshift"))
            ...
            #random_shift = int(np.random.rand() * 512 * grd_noise / 360)
            #print("GRD NOISE: {}".format(random_shift))
            #img_dup = img[:, ((j-random_shift)%512)[:grd_width], :]
            #print(img_dup.shape)
            #print(img_dup)
            # img -= 100.0
            ...
            grd_shift[batch_idx] = 0#random_shift
