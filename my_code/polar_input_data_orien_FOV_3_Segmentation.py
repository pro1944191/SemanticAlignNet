...
class InputData:
    ...
    def next_batch_scan(self, batch_size, grd_noise=360, FOV=360):
        ...
        #batch_sat_polar = np.zeros([batch_size, 128, 512, 6], dtype=np.float32)
        ...
        for i in range(batch_size):
            ...
            #img_s = cv2.rotate(img_s, cv2.ROTATE_180)
            ...
            #img_s = cv2.rotate(img_s, cv2.ROTATE_180)
            combined_image = np.concatenate((img,img_s),axis=2)
            batch_sat_polar[i, :, :, :] = combined_image"""
...
#        print(grd_shift[0])
        ...
        return batch_sat_polar, batch_sat, batch_grd, (np.around(((512-grd_shift)/512*64)%64)).astype(np.int_)
    ...
    def next_pair_batch(self, batch_size, grd_noise=360, FOV=360):
        ...
        #cv2.waitKey(0)
        #print("URL: {}".format(self.id_list[img_idx][0].replace("/","/normal/")))
        #input("WAIT")
        ...
        while True:
            ...
            #print("######################################")
            #print(self.img_root + self.id_list[img_idx][0])
            #print(self.id_list[img_idx][0])
            #print(self.img_root + self.id_list[img_idx][0].replace("/","/normal/"))
            ...
            # SATELLITE POLAR TRANSFORMED SEGMENTED
            #print(self.img_root + self.id_list[img_idx][0])
            #print(self.img_root + self.id_list[img_idx][0].replace("/input","/segmap/output"))
            #print(self.img_root + self.id_list[img_idx][0].replace("/input","/segmap/output"))
            img_s = cv2.imread(self.img_root + self.id_list[img_idx][0].replace("/input","/segmap/output"))
            ...
            combined_image = np.concatenate((img,img_s),axis=2)
            batch_sat_polar[batch_idx, :, :, :] = combined_image
            ...
            # GROUND IMAGE
            #print(self.img_root + self.id_list[img_idx][2].replace("input",""))
            ...
            #Without padd the images
            #img_dup = cv2.resize(img_dup, (256,128))
            #

            # CODE FOR PADD THE IMAGES
            """left = 0
            right = 0
            if img_dup.shape[1]>512:
                left = 0
            else:
                if img_dup.shape[1] % 2 == 0:
                    left = int((512-img_dup.shape[1])/2)
                    right = left
                else:
                    #print(int((512-img_dup.shape[1])/2))
                    left = int((512-img_dup.shape[1])/2)+1
                    right = left-1
            borderType = cv2.BORDER_CONSTANT
            value = [0,0,0]
            dst = cv2.copyMakeBorder(img_dup, 0, 0, left, right, borderType, None, value)"""
            ##########################
            #pil_image = Image.fromarray(np.uint8(img))
            #pil_image2 = Image.fromarray(np.uint8(img_dup))
